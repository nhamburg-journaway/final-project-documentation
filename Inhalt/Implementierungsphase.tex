...
Im Anschluss wurde mit Hilfe des objekt-relationalen Mappers (\acs{ORM}) \acs{EFCore} eine Migration angelegt, die die Tabellen,
Eigenschaften und Beziehungen zwischen Entitäten definiert \cite{MicrosoftEfCoreDocs}. Diese Migration wurde auf die Datenbank angewandt.
Mit diesem Code-First-Ansatz liegt die Kontrolle über die Datenbankstruktur im versionierten Code.
...
Serviceregistrierung in der \acs{DI}-Klasse eine IConnectionFactory als Singleton mit den Applikationseinstellungen registriert.
...
\par\smallskip
Um die verschiedenen Queues zu initialisieren wurde ein QueueInitializer erstellt, welcher als IHostedService implementiert wird,
...
für \textit{RabbitMQ} verwaltet. Hier werden die MainQueues, RetryQueues und \ac{DLQ}'s erstellt, die später für
...
Dafür wurde der HubSpotApiClient implementiert, welcher Daten aus dem Domänen-Modell mappt und damit die Http-Requests ausführt. 
Dieser Client ermöglicht POST und PATCH Anfragen bzw. das Erstellen und Bearbeiten von Kontaktdatensätzen in \textit{HubSpot} \cite{HubSpotApiDocs}.
\par\smallskip
...
\par\smallskip
Um schließlich alle Anforderungen zu erfüllen wurde ein HubSpotRateLimiter implementiert, der dafür sorgt, dass die von \textit{HubSpot}
...
\par\smallskip
Ausschnitte zum HubSpotApiClient und HubSpotRateLimiter sind im \Anhang{app:HubSpot} zu finden.


\subsection{Implementierung der Synchronisationshistorie}
\label{sec:ImplementierungSync}
Um die Synchronisationshistorie dauerhaft speichern und gleichzeitig eine gute Testbarkeit gewährleisten zu können,
wurde die Implementierung in drei Abstraktionsebenen aufgeteilt.
Diese Trennung sorgt dafür, dass fachliche Logik, technische Verarbeitung und direkter Datenbankzugriff voneinander getrennt bleiben.
Abbildung~\ref{fig:DreiEbenenSyncHistory} zeigt den vereinfachten Aufbau dieser Ebenen.
\par\smallskip
...
an Wiederholungen erneut ausführen kann \cite{PollyDocs}.
\par\smallskip
...
und das \textit{single responsibility principle} eingehalten wird \cite{CleanCodeDeveloperGrade}. Zwei Services für die Ressourcen \textit{PersonalInformation} und \textit{CommunicationPreference}
...
\par\smallskip
Als letztes wurde der HubSpotContactService entwickelt. Dieser beinhaltet den HubSpotApiClient und führt die beiden genannten Ressourcen
ein mapping durch, womit diese in eine für \textit{HubSpot} lesbare Form gebracht werden. Fakt ist, dass ein Kontakt nur erstellt werden soll, wenn
