...
Dafür werden drei Event Queues benötigt:
...
sowie eine weitere Queue, über die der \textit{user-service} den erfolgreichen Abschluss einer Synchronisation an den \textit{Nightcrawler} zurückmelden kann.
\par\smallskip
...
Bei einem internen Verrechnungssatz von beispielsweise [70] Euro pro Stunde ergeben sich folgende Entwicklungskosten:

\[
80 \text{ Stunden} \times [70] \text{ Euro/Stunde} = [5600] \text{ Euro}
\]
...
\par\smallskip
Wird angenommen, dass durch die automatisierte Synchronisation wöchentlich [2 bis 3] Stunden manueller Aufwand eingespart werden können,
ergibt sich bei einem internen Stundensatz von [40] Euro ein jährliches Einsparpotenzial von:

\[
[2{,}5] \text{ Stunden/Woche} \times [40] \text{ Euro/Stunde} \times [52] \text{ Wochen} = [5200] \text{ Euro/Jahr}
\]
...
\par\smallskip
Technisch muss die Anwendung drei Event Queues über RabbitMQ verwenden.
...
Insbesondere die Datenqualität, die Nachvollziehbarkeit von Änderungen und die technische Stabilität der Synchronisation werden verbessert.
\par\smallskip
...
Die folgende Matrix stellt einen beispielhaften Vorher-/Nachher-Vergleich dar:
...
\textbf{Automatisierungsgrad}                  & 2  & 2       & 5       \\
\rowcolor{heading}\textbf{Gesamt:}      & \textbf{15} & \textbf{27} & \textbf{70}\\
\rowcolor{odd}\textbf{Nutzwert:}        &           & \textbf{1,8} & \textbf{4,67}\\
\end{tabular}

Aus den beispielhaften Bewertungen ergibt sich, dass der Nutzen der neuen Anwendung vor allem in der zuverlässigeren Verarbeitung,
